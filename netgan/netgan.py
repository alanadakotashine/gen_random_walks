...
class NetGAN:
    ...
    def __init__(self, N, rw_len, walk_generator, generator_layers=[40], discriminator_layers=[30],
                 ...
                 temperature_decay=1-5e-5, seed=15, gpu_id=0, use_gumbel=True, legacy_generator=False, wstate = 1):
        """
        Initialize NetGAN.
        Parameters
        ----------
        ...
        if wstate == 1:
            print('Using lstm w state')
            self.generator_function = self.generator_recurrent
        else:
            print('Using stateless relu')
            print(self.N)
            print(self.rw_len)
            print(self.params['batch_size'])
            self.generator_function = self.generator_recurrent_stateless_relu
        ...
        self.disc_cost_fake = tf.reduce_mean(self.disc_fake)
        self.disc_cost_real = tf.reduce_mean(self.disc_real)
        self.gen_cost_samp = -tf.reduce_mean(self.disc_fake)

        self.disc_cost = self.disc_cost_fake - self.disc_cost_real
        self.gen_cost = self.gen_cost_samp
        ...
        #self.lstm_weights = [v for v in self.gen_params if 'basic_lstm_cell/kernel' in v.name]
        self.w_up = [v for v in self.gen_params if 'W_up' in v.name]
        self.w_down = [v for v in self.gen_params if 'W_Down' in v.name]

        

        #self.lstm_grad = tf.gradients(self.gen_cost,self.lstm_weights)
        self.w_up_grad = tf.gradients(self.gen_cost,self.w_up)
        self.w_down_grad = tf.gradients(self.gen_cost,self.w_down)
    ...
    def generator_recurrent_stateless(self, n_samples, reuse=None, z=None, gumbel=True, legacy=False):
        """
        Generate random walks using LSTM.
        Parameters
        ----------
        ...
        with tf.variable_scope('Generator') as scope:
            ...
            for i in range(self.rw_len):
                if i > 0:
                    tf.get_variable_scope().reuse_variables()
                    state = [(tf.zeros_like(x[0]),tf.zeros_like(x[1])) for x in state]
    ...
    def generator_recurrent_stateless_relu(self, n_samples, reuse=None, z=None, gumbel=True, legacy=False):
        """
        Generate random walks using RNN.
        Parameters
        ----------
        n_samples: int
                   The number of random walks to generate.
        ...
        with tf.variable_scope('Generator') as scope:
            ...
            # initial states h and c are randomly sampled for each lstm cell
            # initial vertex randomly sampled 
            if z is None:
                initial_vertex_noise = make_noise([n_samples, self.noise_dim], self.params['noise_type'])
            else:
                initial_vertex_noise = z
            initial_states = []
            ...
            # Noise preprocessing
            for ix,size in enumerate(self.G_layers):
                intermediate = tf.layers.dense(initial_vertex_noise, self.params['W_Down_Generator_size'], name="Generator.int_{}".format(ix+1),
                                                     reuse=reuse, activation=tf.nn.tanh, use_bias = True)
                inputs = tf.layers.dense(intermediate, self.params['W_Down_Generator_size'], name="Generator.vertex_{}".format(ix+1), reuse=reuse,
            ...
            for i in range(self.rw_len):
                if i > 0:
                    ...
                    #state = [(tf.zeros_like(x[0]),tf.zeros_like(x[1])) for x in state]
                for ix,size in enumerate(self.G_layers):
                    inter = tf.layers.dense(inputs,size,name="Generator.inter_layer_{}".format(ix+1), reuse=reuse,
                                        activation=tf.nn.relu)
                    inputs = tf.layers.dense(inter,size,name="Generator.layer_{}".format(ix+1), reuse=reuse,
                                        activation=tf.nn.tanh)


                #take the input, which is a 128 dimensional vector. This is now mapped to 
                #an n-dimensional vector, so a probability of which vertex this actually is. 

                # Blow up to dimension N using W_up
                output_bef = tf.matmul(inputs, self.W_up) + self.b_W_up
    ...
    def train(self, A_orig, val_ones, val_zeros,  max_iters=50000, stopping=None, eval_transitions=15e4,
        ...
        for _it in range(max_iters):
            ...
            gen_loss, _, gen_loss_samples, gen_w_down_grad, gen_w_up_grad = self.session.run([self.gen_cost, self.gen_train_op, self.gen_cost_samp, self.w_down_grad, self.w_up_grad],
            ...
            for _ in range(self.params['disc_iters']):
                disc_loss, _ , disc_loss_real, disc_loss_fake = self.session.run(
                    [self.disc_cost, self.disc_train_op, self.disc_cost_real, self.disc_cost_fake],
                    feed_dict={self.tau: temperature}
                )
            ...
            gen_losses_samp.append(gen_loss_samples)
            disc_losses_real.append(disc_loss_real)
            disc_losses_fake.append(disc_loss_fake)
            ...
            if plot_every > 0 and (_it) % plot_every == 0 and _it > 0:
                ...
                plt.legend()
                plt.savefig('netgan/plots/losses_{}.pdf'.format(_it))
                plt.gcf().clear()

                plt.plot(disc_losses_fake, label="Critic loss fake")
                plt.plot(disc_losses_real, label="Critic loss real")
                plt.plot(gen_losses_samp, label="Generator loss")
                plt.legend()
                plt.savefig('netgan/plots/costs_{}.pdf'.format(_it))
                plt.gcf().clear()

                np.savetxt('netgan/plots/samples_{}.txt'.format(_it),gr_dense)
                np.savetxt('netgan/plots/sampled_graph_{}.txt'.format(_it),_graph)
                #np.savetxt('netgan/plots/lstm_weights_grads_{}.txt'.format(_it),gen_lstm_grad[0])
                np.savetxt('netgan/plots/w_up_grads_{}.txt'.format(_it),gen_w_up_grad[0])
                np.savetxt('netgan/plots/w_down_grads_{}.txt'.format(_it),gen_w_down_grad[0])
