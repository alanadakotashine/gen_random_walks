...
if __name__ == "__main__":
    ...
    for i in range(start,k+1):
        ...
        barbell_edge_density_2.append(X_2[3][99])
       
        num_edges_crossing_1.append(X_1[np.ix_(range(50),range(50,100))].sum())
        num_edges_crossing_2.append(X_2[np.ix_(range(50),range(50,100))].sum())
    ...
    plt.plot(index,barbell_edge_density_1,label=label_1)
    plt.plot(index,barbell_edge_density_2,label=label_2)
    plt.xlabel('Num Training Iterations')
    plt.ylabel('Edge Density Expectation Matrix')
    plt.legend(loc='best')
    plt.savefig('plots/'+name+'_edgeDensity_expected_comp.pdf')
    plt.gcf().clear()


    plt.plot(index,num_edges_crossing_1,label=label_1)
    plt.plot(index,num_edges_crossing_2,label=label_2)
    plt.legend(loc='best')
    plt.xlabel('Num Training Iterations')
    plt.ylabel('Num Edges Crossing')
    plt.savefig('plots/'+name+'_num_edges_crossing_expected_comp.pdf')
    plt.gcf().clear()
